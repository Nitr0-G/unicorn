...
#define cpu_cc_dst     (tcg_ctx->cpu_cc_dst)
#define cpu_cc_src     (tcg_ctx->cpu_cc_src)
#define cpu_cc_src2    (tcg_ctx->cpu_cc_src2)
#define cpu_eip        (tcg_ctx->cpu_eip)
#define cpu_cc_op      (tcg_ctx->cpu_cc_op)
#define cpu_regs       (tcg_ctx->cpu_regs)
#define cpu_seg_base   (tcg_ctx->cpu_seg_base)
#define cpu_bndl       (tcg_ctx->cpu_bndl)
#define cpu_bndu       (tcg_ctx->cpu_bndu)
#define cpu_env        (tcg_ctx->cpu_env)
...
static void clear_rex_prefix(DisasContext *s, int *prefixes)
{
#ifdef TARGET_X86_64
    if (CODE64(s)) {
        *prefixes &= ~PREFIX_REX;
...
#define CC_OP_LIVE_BWLQ(OP, LIVE) \
    [CC_OP_##OP##B] = LIVE,       \
    [CC_OP_##OP##W] = LIVE,       \
    [CC_OP_##OP##L] = LIVE,       \
    [CC_OP_##OP##Q] = LIVE
...
static const uint8_t cc_op_live[CC_OP_NB] = {
    ...
    CC_OP_LIVE_BWLQ(MUL, USES_CC_DST | USES_CC_SRC),
    CC_OP_LIVE_BWLQ(ADD, USES_CC_DST | USES_CC_SRC),
    CC_OP_LIVE_BWLQ(ADC, USES_CC_DST | USES_CC_SRC | USES_CC_SRC2),
    CC_OP_LIVE_BWLQ(SUB, USES_CC_DST | USES_CC_SRC | USES_CC_SRCT),
    CC_OP_LIVE_BWLQ(SBB, USES_CC_DST | USES_CC_SRC | USES_CC_SRC2),
    CC_OP_LIVE_BWLQ(LOGIC, USES_CC_DST),
    CC_OP_LIVE_BWLQ(INC, USES_CC_DST | USES_CC_SRC),
    CC_OP_LIVE_BWLQ(DEC, USES_CC_DST | USES_CC_SRC),
    CC_OP_LIVE_BWLQ(SHL, USES_CC_DST | USES_CC_SRC),
    CC_OP_LIVE_BWLQ(SAR, USES_CC_DST | USES_CC_SRC),
    CC_OP_LIVE_BWLQ(BMILG, USES_CC_DST | USES_CC_SRC),
    CC_OP_LIVE_BWLQ(BLSI, USES_CC_DST | USES_CC_SRC),
...
static TCGv gen_op_deposit_reg_v(DisasContext *s, MemOp ot, int reg, TCGv dest, TCGv t0)
{
    ...
    case MO_8:
        if (byte_reg_is_xH(s, reg)) {
            ...
            tcg_gen_deposit_tl(tcg_ctx, dest, cpu_regs[reg - 4], t0, 8, 8);
            return cpu_regs[reg - 4];
        }
        dest = dest ? dest : cpu_regs[reg];
        tcg_gen_deposit_tl(tcg_ctx, dest, cpu_regs[reg], t0, 0, 8);
        break;
    case MO_16:
        dest = dest ? dest : cpu_regs[reg];
        tcg_gen_deposit_tl(tcg_ctx, dest, cpu_regs[reg], t0, 0, 16);
        break;
    case MO_32:
...
{
    TCGContext *tcg_ctx = s->uc->tcg_ctx;
    tcg_gen_mov_tl(tcg_ctx, cpu_eip, dest);
    s->pc_save = -1;
}
...
static void gen_update_eip_cur(DisasContext *s)
{
    ...
    assert(s->pc_save != -1);
    if (TARGET_TB_PCREL) {
        tcg_gen_addi_tl(tcg_ctx, cpu_eip, cpu_eip, s->base.pc_next - s->pc_save);
    } else if (CODE64(s)) {
        tcg_gen_movi_tl(tcg_ctx, cpu_eip, s->base.pc_next);
    } else {
        tcg_gen_movi_tl(tcg_ctx, cpu_eip, (uint32_t)(s->base.pc_next - s->cs_base));
    }
...
static void gen_update_eip_next(DisasContext *s)
{
    ...
    assert(s->pc_save != -1);
    if (TARGET_TB_PCREL) {
        tcg_gen_addi_tl(tcg_ctx, cpu_eip, cpu_eip, s->pc - s->pc_save);
    } else if (CODE64(s)) {
        tcg_gen_movi_tl(tcg_ctx, cpu_eip, s->pc);
    } else {
        tcg_gen_movi_tl(tcg_ctx, cpu_eip, (uint32_t)(s->pc - s->cs_base));
    }
    s->pc_save = s->pc;
}
...
{
    TCGContext *tcg_ctx = s->uc->tcg_ctx;
    return tcg_constant_i32(tcg_ctx, cur_insn_len(s));
}
...
static TCGv_i32 eip_next_i32(DisasContext *s)
{
     ...
     */
    if (CODE64(s)) {
        return tcg_constant_i32(tcg_ctx, -1);
    }
    if (TARGET_TB_PCREL) {
        TCGv_i32 ret = tcg_temp_new_i32(tcg_ctx);
        tcg_gen_trunc_tl_i32(tcg_ctx, ret, cpu_eip);
        tcg_gen_addi_i32(tcg_ctx, ret, ret, s->pc - s->pc_save);
        return ret;
    } else {
        return tcg_constant_i32(tcg_ctx, s->pc - s->cs_base);
    }
}
...
static TCGv eip_next_tl(DisasContext *s)
{
    ...
    assert(s->pc_save != -1);
    if (TARGET_TB_PCREL) {
        TCGv ret = tcg_temp_new(tcg_ctx);
        tcg_gen_addi_tl(tcg_ctx, ret, cpu_eip, s->pc - s->pc_save);
        return ret;
    } else if (CODE64(s)) {
        return tcg_constant_tl(tcg_ctx, s->pc);
    } else {
        return tcg_constant_tl(tcg_ctx, (uint32_t)(s->pc - s->cs_base));
    }
}
...
static TCGv eip_cur_tl(DisasContext *s)
{
    ...
    if (TARGET_TB_PCREL) {
        TCGv ret = tcg_temp_new(tcg_ctx);
        tcg_gen_addi_tl(tcg_ctx, ret, cpu_eip, s->base.pc_next - s->pc_save);
        return ret;
    } else if (CODE64(s)) {
        return tcg_constant_tl(tcg_ctx, s->base.pc_next);
    } else {
        return tcg_constant_tl(tcg_ctx, (uint32_t)(s->base.pc_next - s->cs_base));
    }
}
...
{
    TCGContext *tcg_ctx = s->uc->tcg_ctx;
    tcg_gen_ld32s_tl(tcg_ctx, s->T0, cpu_env, offsetof(CPUX86State, df));
    tcg_gen_shli_tl(tcg_ctx, s->T0, s->T0, ot);
};

static TCGv gen_ext_tl(TCGContext *tcg_ctx, TCGv dst, TCGv src,
                       MemOp size, bool sign)
{
    switch (size) {
    case MO_8:
        if (sign) {
...
static void gen_op_j_ecx(DisasContext *s, TCGCond cond, TCGLabel *label1)
{
    ...
    gen_extu(tcg_ctx, s->aflag, s->tmp0);
    tcg_gen_brcondi_tl(tcg_ctx, cond, s->tmp0, 0, label1);
}
...
static void gen_helper_in_func(TCGContext *tcg_ctx, MemOp ot, TCGv v,
                               TCGv_i32 n)
{
    switch (ot) {
    case MO_8:
        gen_helper_inb(tcg_ctx, v, cpu_env, n);
        break;
    case MO_16:
        gen_helper_inw(tcg_ctx, v, cpu_env, n);
        break;
    case MO_32:
        gen_helper_inl(tcg_ctx, v, cpu_env, n);
        break;
    default:
        tcg_abort();
    }
...
static void gen_helper_out_func(TCGContext *tcg_ctx, MemOp ot, TCGv_i32 v,
                                TCGv_i32 n)
{
    switch (ot) {
    case MO_8:
        gen_helper_outb(tcg_ctx, cpu_env, v, n);
        break;
    case MO_16:
        gen_helper_outw(tcg_ctx, cpu_env, v, n);
        break;
    case MO_32:
        gen_helper_outl(tcg_ctx, cpu_env, v, n);
        break;
    default:
        tcg_abort();
    }
                         ...
                         uint32_t svm_flags)
{
    ...
    if (PE(s) && (CPL(s) > IOPL(s) || VM86(s))) {
        gen_helper_check_io(tcg_ctx, cpu_env, port, tcg_constant_i32(tcg_ctx, 1 << ot));
    }
    if (GUEST(s)) {
        gen_update_cc_op(s);
        ...
        gen_helper_svm_check_io(tcg_ctx, cpu_env, port,
                                tcg_constant_i32(tcg_ctx, svm_flags),
                                cur_insn_len_i32(s));
    }
    return true;
#endif
...
static CCPrepare gen_prepare_eflags_c(DisasContext *s, TCGv reg)
{
    ...
    case CC_OP_SUBB: case CC_OP_SUBW: case CC_OP_SUBL: case CC_OP_SUBQ:
        ...
        t1 = gen_ext_tl(tcg_ctx, s->tmp0, cpu_cc_src, size, false);
...
static CCPrepare gen_prepare_cc(DisasContext *s, int b, TCGv reg)
{
    ...
    case CC_OP_SUBB: case CC_OP_SUBW: case CC_OP_SUBL: case CC_OP_SUBQ:
        ...
        case JCC_BE:
            ...
            t0 = gen_ext_tl(tcg_ctx, s->tmp0, cpu_cc_src, size, false);
        ...
        fast_jcc_l:
            ...
            t0 = gen_ext_tl(tcg_ctx, s->tmp0, cpu_cc_src, size, true);
...
#else
        TCGv_i32 t_size = tcg_constant_i32(tcg_ctx, 1 << ot);
        TCGv t_next = eip_next_tl(s);
        gen_helper_bpt_io(tcg_ctx, cpu_env, t_port, t_size, t_next);
...
static void gen_helper_fp_arith_ST0_FT0(TCGContext *tcg_ctx, int op)
{
    switch (op) {
    case 0:
        gen_helper_fadd_ST0_FT0(tcg_ctx, cpu_env);
        break;
    case 1:
        gen_helper_fmul_ST0_FT0(tcg_ctx, cpu_env);
        break;
    case 2:
    ...
    case 3:
        ...
        break;
    case 4:
        gen_helper_fsub_ST0_FT0(tcg_ctx, cpu_env);
        break;
    case 5:
        gen_helper_fsubr_ST0_FT0(tcg_ctx, cpu_env);
        break;
    case 6:
        gen_helper_fdiv_ST0_FT0(tcg_ctx, cpu_env);
        break;
    case 7:
        gen_helper_fdivr_ST0_FT0(tcg_ctx, cpu_env);
        break;
    }
}
...
static void gen_helper_fp_arith_STN_ST0(TCGContext *tcg_ctx, int op, int opreg)
{
    ...
    switch (op) {
    case 0:
        gen_helper_fadd_STN_ST0(tcg_ctx, cpu_env, tmp);
        break;
    case 1:
        gen_helper_fmul_STN_ST0(tcg_ctx, cpu_env, tmp);
        break;
    case 4:
        gen_helper_fsubr_STN_ST0(tcg_ctx, cpu_env, tmp);
        break;
    case 5:
        gen_helper_fsub_STN_ST0(tcg_ctx, cpu_env, tmp);
        break;
    case 6:
        gen_helper_fdivr_STN_ST0(tcg_ctx, cpu_env, tmp);
        break;
    case 7:
        gen_helper_fdiv_STN_ST0(tcg_ctx, cpu_env, tmp);
        break;
    }
}
...
static void gen_exception(DisasContext *s, int trapno)
{
    ...
    gen_update_cc_op(s);
    gen_update_eip_cur(s);
    gen_helper_raise_exception(tcg_ctx, cpu_env, tcg_const_i32(tcg_ctx, trapno));
...
static void gen_op(DisasContext *s1, int op, MemOp ot, int d)
{
    ...
    case OP_SUBL:
        ...
        if (HOOK_EXISTS_BOUNDED(s1->uc, UC_HOOK_TCG_OPCODE,
                                s1->base.pc_next)) {
            struct hook *hook;
            HOOK_FOREACH_VAR_DECLARE;

            HOOK_FOREACH(s1->uc, hook, UC_HOOK_TCG_OPCODE) {
                if (hook->to_delete) {
                    continue;
                }
                if (hook->op == UC_TCG_OP_SUB &&
                    (hook->op_flags & UC_TCG_OP_FLAG_DIRECT)) {
                    gen_uc_traceopcode(tcg_ctx, hook, (TCGv_i64)s1->T0,
                                       (TCGv_i64)s1->T1,
                                       1 << ((ot & MO_SIZE) + 3), s1->uc,
                                       s1->base.pc_next);
                }
            }
        }
    ...
    case OP_XORL:
        ...
        break;
    case OP_CMPL:
        tcg_gen_mov_tl(tcg_ctx, cpu_cc_src, s1->T1);
        tcg_gen_mov_tl(tcg_ctx, s1->cc_srcT, s1->T0);
        tcg_gen_sub_tl(tcg_ctx, cpu_cc_dst, s1->T0, s1->T1);
        if (HOOK_EXISTS_BOUNDED(s1->uc, UC_HOOK_TCG_OPCODE,
                                s1->base.pc_next)) {
            struct hook *hook;
            HOOK_FOREACH_VAR_DECLARE;
            ...
            HOOK_FOREACH(s1->uc, hook, UC_HOOK_TCG_OPCODE) {
                ...
                if (hook->op == UC_TCG_OP_SUB &&
                    (hook->op_flags & UC_TCG_OP_FLAG_CMP)) {
                    gen_uc_traceopcode(tcg_ctx, hook, (TCGv_i64)s1->T0,
                                       (TCGv_i64)s1->T1,
                                       1 << ((ot & MO_SIZE) + 3), s1->uc,
                                       s1->base.pc_next);
                           ...
                           int is_right)
{
    ...
    if (is_right) {
        switch (ot) {
        case MO_8:
            gen_helper_rcrb(tcg_ctx, s->T0, cpu_env, s->T0, s->T1);
            break;
        case MO_16:
            gen_helper_rcrw(tcg_ctx, s->T0, cpu_env, s->T0, s->T1);
            break;
        case MO_32:
            gen_helper_rcrl(tcg_ctx, s->T0, cpu_env, s->T0, s->T1);
            break;
#ifdef TARGET_X86_64
        case MO_64:
            gen_helper_rcrq(tcg_ctx, s->T0, cpu_env, s->T0, s->T1);
            break;
#endif
        default:
            tcg_abort();
        ...
        switch (ot) {
        case MO_8:
            gen_helper_rclb(tcg_ctx, s->T0, cpu_env, s->T0, s->T1);
            break;
        case MO_16:
            gen_helper_rclw(tcg_ctx, s->T0, cpu_env, s->T0, s->T1);
            break;
        case MO_32:
            gen_helper_rcll(tcg_ctx, s->T0, cpu_env, s->T0, s->T1);
            break;
#ifdef TARGET_X86_64
        case MO_64:
            gen_helper_rclq(tcg_ctx, s->T0, cpu_env, s->T0, s->T1);
            break;
#endif
        default:
            tcg_abort();
...
static inline uint8_t x86_ldub_code(CPUX86State *env, DisasContext *s)
{
    ...
    return translator_ldub(tcg_ctx, env, advance_pc(env, s, 1));
}
...
static inline int16_t x86_ldsw_code(CPUX86State *env, DisasContext *s)
{
    ...
    return translator_ldsw(tcg_ctx, env, advance_pc(env, s, 2));
}
...
static inline uint16_t x86_lduw_code(CPUX86State *env, DisasContext *s)
{
    ...
    return translator_lduw(tcg_ctx, env, advance_pc(env, s, 2));
}
...
static inline uint32_t x86_ldl_code(CPUX86State *env, DisasContext *s)
{
    ...
    return translator_ldl(tcg_ctx, env, advance_pc(env, s, 4));
}
...
static inline uint64_t x86_ldq_code(CPUX86State *env, DisasContext *s)
{
    ...
    return translator_ldq(tcg_ctx, env, advance_pc(env, s, 8));
}
#endif
...
static TCGv gen_lea_modrm_1(DisasContext *s, AddressParts a, bool is_vsib)
{
    ...
    if (!ea) {
        if (TARGET_TB_PCREL && a.base == -2) {
            ...
            tcg_gen_addi_tl(tcg_ctx, s->A0, cpu_eip, a.disp - s->pc_save);
        } else {
                      ...
                      TCGCond cond, TCGv_i64 bndv)
{
    ...
    gen_helper_bndck(tcg_ctx, cpu_env, s->tmp2_i32);
}
...
static void gen_svm_check_intercept(DisasContext *s, uint32_t type)
{
    ...
    gen_helper_svm_check_intercept(tcg_ctx, cpu_env, tcg_constant_i32(tcg_ctx, type));
}
...
static void gen_interrupt(DisasContext *s, int intno)
{
    ...
    gen_update_cc_op(s);
    gen_update_eip_cur(s);
    gen_helper_raise_interrupt(tcg_ctx, cpu_env, tcg_constant_i32(tcg_ctx, intno),
                               cur_insn_len_i32(s));
    s->base.is_jmp = DISAS_NORETURN;
}
...
static void gen_debug(DisasContext *s, target_ulong cur_eip)
{
    ...
    gen_update_cc_op(s);
    gen_update_eip_cur(s);
    gen_helper_debug(tcg_ctx, cpu_env);
    s->base.is_jmp = DISAS_NORETURN;
}
...
static void gen_set_eflags(DisasContext *s, target_ulong mask)
{
    ...
    tcg_gen_ld_tl(tcg_ctx, t, cpu_env, offsetof(CPUX86State, eflags));
    tcg_gen_ori_tl(tcg_ctx, t, t, mask);
    tcg_gen_st_tl(tcg_ctx, t, cpu_env, offsetof(CPUX86State, eflags));
...
static void gen_reset_eflags(DisasContext *s, target_ulong mask)
{
    ...
    tcg_gen_ld_tl(tcg_ctx, t, cpu_env, offsetof(CPUX86State, eflags));
    tcg_gen_andi_tl(tcg_ctx, t, t, ~mask);
    tcg_gen_st_tl(tcg_ctx, t, cpu_env, offsetof(CPUX86State, eflags));
    tcg_temp_free(tcg_ctx, t);
}
...
static void gen_jmp_rel(DisasContext *s, MemOp ot, int diff, int tb_num)
{
    ...
    if (TARGET_TB_PCREL) {
        tcg_gen_addi_tl(tcg_ctx, cpu_eip, cpu_eip, new_pc - s->pc_save);
        /*
        ...
        if (!use_goto_tb || !is_same_page(&s->base, new_pc)) {
            tcg_gen_andi_tl(tcg_ctx, cpu_eip, cpu_eip, mask);
            use_goto_tb = false;
        }
    } else if (!CODE64(s)) {
    ...
    if (use_goto_tb && translator_use_goto_tb(&s->base, new_pc)) {
        ...
        if (!TARGET_TB_PCREL) {
            tcg_gen_movi_tl(tcg_ctx, cpu_eip, new_eip);
        }
...
static inline void gen_sto_env_A0(DisasContext *s, int offset, bool align)
{
    ...
    tcg_gen_qemu_st_i64(tcg_ctx, s->tmp1_i64, s->A0, mem_index,
                        MO_LEUQ | (align ? MO_ALIGN_16 : 0));
    tcg_gen_addi_tl(tcg_ctx, s->tmp0, s->A0, 8);
    tcg_gen_ld_i64(tcg_ctx, s->tmp1_i64, cpu_env, offset + offsetof(XMMReg, XMM_Q(1)));
...
static void gen_ldy_env_A0(DisasContext *s, int offset, bool align)
{
    ...
    tcg_gen_addi_tl(tcg_ctx, s->tmp0, s->A0, 8);
    tcg_gen_qemu_ld_i64(tcg_ctx, s->tmp1_i64, s->tmp0, mem_index, MO_LEUQ);
    tcg_gen_st_i64(tcg_ctx, s->tmp1_i64, cpu_env, offset + offsetof(YMMReg, YMM_Q(1)));

    tcg_gen_addi_tl(tcg_ctx, s->tmp0, s->A0, 16);
    tcg_gen_qemu_ld_i64(tcg_ctx, s->tmp1_i64, s->tmp0, mem_index, MO_LEUQ);
    tcg_gen_st_i64(tcg_ctx, s->tmp1_i64, cpu_env, offset + offsetof(YMMReg, YMM_Q(2)));
    tcg_gen_addi_tl(tcg_ctx, s->tmp0, s->A0, 24);
    tcg_gen_qemu_ld_i64(tcg_ctx, s->tmp1_i64, s->tmp0, mem_index, MO_LEUQ);
    tcg_gen_st_i64(tcg_ctx, s->tmp1_i64, cpu_env, offset + offsetof(YMMReg, YMM_Q(3)));
}
...
static void gen_sty_env_A0(DisasContext *s, int offset, bool align)
{
    ...
    tcg_gen_qemu_st_i64(tcg_ctx, s->tmp1_i64, s->A0, mem_index,
                        MO_LEUQ | (align ? MO_ALIGN_32 : 0));
    tcg_gen_addi_tl(tcg_ctx, s->tmp0, s->A0, 8);
    tcg_gen_ld_i64(tcg_ctx, s->tmp1_i64, cpu_env, offset + offsetof(YMMReg, YMM_Q(1)));
    tcg_gen_qemu_st_i64(tcg_ctx, s->tmp1_i64, s->tmp0, mem_index, MO_LEUQ);
    tcg_gen_addi_tl(tcg_ctx, s->tmp0, s->A0, 16);
    tcg_gen_ld_i64(tcg_ctx, s->tmp1_i64, cpu_env, offset + offsetof(YMMReg, YMM_Q(2)));
    tcg_gen_qemu_st_i64(tcg_ctx, s->tmp1_i64, s->tmp0, mem_index, MO_LEUQ);
    tcg_gen_addi_tl(tcg_ctx, s->tmp0, s->A0, 24);
    tcg_gen_ld_i64(tcg_ctx, s->tmp1_i64, cpu_env, offset + offsetof(YMMReg, YMM_Q(3)));
    tcg_gen_qemu_st_i64(tcg_ctx, s->tmp1_i64, s->tmp0, mem_index, MO_LEUQ);
}
...
static bool disas_insn(DisasContext *s, CPUState *cpu)
{
    ...
    if (HOOK_EXISTS_BOUNDED(s->uc, UC_HOOK_CODE, pc_start)) {
        ...
        insn_hook = true;
        gen_uc_tracecode(tcg_ctx, 0xf1f1f1f1, UC_HOOK_CODE_IDX, s->uc,
                         pc_start);
        check_exit_request(tcg_ctx);
        gen_compute_eflags(s);
    }
    ...
    case 2:
        ...
        /* END TODO */
        s->base.num_insns--;
        tcg_remove_ops_after(tcg_ctx, s->prev_insn_end);
 ...
 next_byte:
    ...
    case 0xf7:
        ...
        case 3: /* neg */
            if (s->prefix & PREFIX_LOCK) {
                ...
                tcg_gen_neg_tl(tcg_ctx, s->T0, t0);
    ...
    case 0x1b1: /* cmpxchg Ev, Gv */
        {
            ...
            tcg_gen_mov_tl(tcg_ctx, cmpv, cpu_regs[R_EAX]);
            ...
            if (s->prefix & PREFIX_LOCK) {
                ...
                if (mod == 3) {
                    ...
                    tcg_gen_movcond_tl(tcg_ctx, TCG_COND_EQ, dest, oldv, cmpv, newv, dest);
                } else {
                    gen_lea_modrm(env, s, modrm);
	     ...
	     */
            ...
            tcg_gen_movcond_tl(tcg_ctx, TCG_COND_EQ, dest, oldv, cmpv, dest, newv);
    ...
    case 0xd8: case 0xd9: case 0xda: case 0xdb: case 0xdc: case 0xdd: case 0xde: case 0xdf:
        {
            ...
            if (mod != 3) {
                ...
                switch (op) {
                case 0x0: case 0x1: case 0x2: case 0x3: case 0x4: case 0x5: case 0x6: case 0x7: /* fxxxs */
                ...
                case 0x0c: /* fldenv mem */
                    gen_helper_fldenv(tcg_ctx, cpu_env, s->A0,
                ...
                case 0x0e: /* fnstenv mem */
                    gen_helper_fstenv(tcg_ctx, cpu_env, s->A0,
                                      tcg_const_i32(tcg_ctx, dflag - 1));
                    update_fip = update_fdp = false;
                    break;
                ...
                case 0x2c: /* frstor mem */
                    gen_helper_frstor(tcg_ctx, cpu_env, s->A0,
                                      tcg_const_i32(tcg_ctx, dflag - 1));
                    update_fip = update_fdp = false;
                    break;
                case 0x2e: /* fnsave mem */
                    gen_helper_fsave(tcg_ctx, cpu_env, s->A0,
                                     tcg_const_i32(tcg_ctx, dflag - 1));
                    update_fip = update_fdp = false;
                    break;
                ...
                case 0x0f: /* grp d9/7 */
                    ...
                    break;
                case 0x00: case 0x01: case 0x4: case 0x5: case 0x6: case 0x7: /* fxxx st, sti */
                ...
                case 0x30: case 0x31: case 0x34: case 0x35: case 0x36: case 0x37: /* fxxxp sti, st */
                    {
                        ...
                        if (op >= 0x20) {
                            ...
                            gen_helper_fmov_FT0_STN(tcg_ctx, cpu_env,
                                                    tcg_const_i32(tcg_ctx, opreg));
                            gen_helper_fp_arith_ST0_FT0(tcg_ctx, op1);
                        }
                    }
                    break;
    ...
    case 0x6d:
        ot = mo_b_d32(b, dflag);
        tcg_gen_trunc_tl_i32(tcg_ctx, s->tmp2_i32, cpu_regs[R_EDX]);
        tcg_gen_ext16u_i32(tcg_ctx, s->tmp2_i32, s->tmp2_i32);
    ...
    case 0xcf: /* iret */
        ...
        if (!PE(s) || VM86(s)) {
            ...
            gen_helper_iret_protected(tcg_ctx, cpu_env, tcg_constant_i32(tcg_ctx, dflag - 1),
                                      eip_next_i32(s));
        }
        set_cc_op(s, CC_OP_EFLAGS);
        s->base.is_jmp = DISAS_EOB_ONLY;
    ...
    case 0x9e: /* sahf */
        if (CODE64(s) && !(s->cpuid_ext3_features & CPUID_EXT3_LAHF_LM))
            goto illegal_op;
        tcg_gen_shri_tl(tcg_ctx, s->T0, cpu_regs[R_EAX], 8);
        gen_compute_eflags(s);
    ...
    case 0x9f: /* lahf */
        ...
        tcg_gen_deposit_tl(tcg_ctx, cpu_regs[R_EAX], cpu_regs[R_EAX], s->T0, 8, 8);
        break;
    case 0xf5: /* cmc */
        gen_compute_eflags(s);
    ...
    case 0x90: /* nop */
        ...
        if (prefixes & PREFIX_REPZ) {
            gen_update_cc_op(s);
            gen_update_eip_cur(s);
            gen_helper_pause(tcg_ctx, cpu_env, cur_insn_len_i32(s));
            s->base.is_jmp = DISAS_NORETURN;
        }
        break;
    case 0x9b: /* fwait */
    ...
    case 0xce: /* into */
        ...
        gen_update_cc_op(s);
        gen_update_eip_cur(s);
        gen_helper_into(tcg_ctx, cpu_env, cur_insn_len_i32(s));
        break;
#ifdef WANT_ICEBP
    case 0xf1: /* icebp (undocumented, exits to external debugger) */
        gen_svm_check_intercept(s, SVM_EXIT_ICEBP);
        gen_debug(s, s->base.pc_next - s->cs_base);
        break;
#endif
    case 0xfa: /* cli */
        if (check_iopl(s)) {
    ...
    case 0xf4: /* hlt */
        if (check_cpl0(s)) {
            gen_update_cc_op(s);
            gen_update_eip_cur(s);
            gen_helper_hlt(tcg_ctx, cpu_env, cur_insn_len_i32(s));
            s->base.is_jmp = DISAS_NORETURN;
        }
        break;
    case 0x100:
    ...
    case 0x101:
        ...
        case 0xc9: /* mwait */
            ...
            gen_update_cc_op(s);
            gen_update_eip_cur(s);
            gen_helper_mwait(tcg_ctx, cpu_env, cur_insn_len_i32(s));
            s->base.is_jmp = DISAS_NORETURN;
            break;
        ...
        case 0xdf: /* INVLPGA */
            ...
            if (s->aflag == MO_64) {
                ...
                tcg_gen_ext32u_tl(tcg_ctx, s->A0, cpu_regs[R_EAX]);
            }
            gen_helper_flush_page(tcg_ctx, cpu_env, s->A0);
            s->base.is_jmp = DISAS_EOB_NEXT;
            break;
        ...
        CASE_MODRM_OP(6): /* lmsw */
             ...
             */
            tcg_gen_ld_tl(tcg_ctx, s->T1, cpu_env, offsetof(CPUX86State, cr[0]));
            tcg_gen_andi_tl(tcg_ctx, s->T0, s->T0, 0xf);
            tcg_gen_andi_tl(tcg_ctx, s->T1, s->T1, ~0xe);
            tcg_gen_or_tl(tcg_ctx, s->T0, s->T0, s->T1);
            gen_helper_write_crN(tcg_ctx, cpu_env, tcg_constant_i32(tcg_ctx, 0), s->T0);
            s->base.is_jmp = DISAS_EOB_NEXT;
            break;
    ...
    case 0x11a:
        ...
        if (s->flags & HF_MPX_EN_MASK) {
            ...
            if (prefixes & PREFIX_REPZ) {
                ...
                tcg_gen_not_i64(tcg_ctx, notu, cpu_bndu[reg]);
                ...
                if (mod == 3) {
                    ...
                    if (s->flags & HF_MPX_IU_MASK) {
                        tcg_gen_mov_i64(tcg_ctx, cpu_bndl[reg], cpu_bndl[reg2]);
                        tcg_gen_mov_i64(tcg_ctx, cpu_bndu[reg], cpu_bndu[reg2]);
                    }
                } else {
                    gen_lea_modrm(env, s, modrm);
                    if (CODE64(s)) {
                ...
                if (CODE64(s)) {
                    gen_helper_bndldx64(tcg_ctx, cpu_bndl[reg], cpu_env, s->A0, s->T0);
                    tcg_gen_ld_i64(tcg_ctx, cpu_bndu[reg], cpu_env,
                                   offsetof(CPUX86State, mmx_t0.MMX_Q(0)));
                } else {
                    gen_helper_bndldx32(tcg_ctx, cpu_bndu[reg], cpu_env, s->A0, s->T0);
                    tcg_gen_ext32u_i64(tcg_ctx, cpu_bndl[reg], cpu_bndu[reg]);
                    tcg_gen_shri_i64(tcg_ctx, cpu_bndu[reg], cpu_bndu[reg], 32);
                }
                gen_set_hflag(s, HF_MPX_IU_MASK);
            }
        }
    ...
    case 0x11b:
        ...
        if (s->flags & HF_MPX_EN_MASK) {
            ...
            if (mod != 3 && (prefixes & PREFIX_REPZ)) {
                ...
                AddressParts a = gen_lea_modrm_0(env, s, modrm);
                if (a.base >= 0) {
                    tcg_gen_extu_tl_i64(tcg_ctx, cpu_bndl[reg], cpu_regs[a.base]);
                    if (!CODE64(s)) {
                        tcg_gen_ext32u_i64(tcg_ctx, cpu_bndl[reg], cpu_bndl[reg]);
                    }
                } else if (a.base == -1) {
                    /* no base register has lower bound of 0 */
                    tcg_gen_movi_i64(tcg_ctx, cpu_bndl[reg], 0);
                } else {
                ...
                tcg_gen_extu_tl_i64(tcg_ctx, cpu_bndu[reg], s->A0);
                /* bnd registers are now in-use */
                gen_set_hflag(s, HF_MPX_IU_MASK);
                break;
            } else if (prefixes & PREFIX_REPNZ) {
                ...
                if (mod == 3) {
                    ...
                    if (s->flags & HF_MPX_IU_MASK) {
                        tcg_gen_mov_i64(tcg_ctx, cpu_bndl[reg2], cpu_bndl[reg]);
                        tcg_gen_mov_i64(tcg_ctx, cpu_bndu[reg2], cpu_bndu[reg]);
                    }
                } else {
                    gen_lea_modrm(env, s, modrm);
                    if (CODE64(s)) {
                        tcg_gen_qemu_st_i64(tcg_ctx, cpu_bndl[reg], s->A0,
                                            s->mem_index, MO_LEUQ);
                        tcg_gen_addi_tl(tcg_ctx, s->A0, s->A0, 8);
                        tcg_gen_qemu_st_i64(tcg_ctx, cpu_bndu[reg], s->A0,
                                            s->mem_index, MO_LEUQ);
                    } else {
                        tcg_gen_qemu_st_i64(tcg_ctx, cpu_bndl[reg], s->A0,
                                            s->mem_index, MO_LEUL);
    ...
    case 0x122: /* mov crN, reg */
        ...
        if (b & 2) {
            ...
            gen_op_mov_v_reg(s, ot, s->T0, rm);
            gen_helper_write_crN(tcg_ctx, cpu_env, tcg_constant_i32(tcg_ctx, reg), s->T0);
            s->base.is_jmp = DISAS_EOB_NEXT;
        } else {
            gen_svm_check_intercept(s, SVM_EXIT_READ_CR0 + reg);
            gen_helper_read_crN(tcg_ctx, s->T0, cpu_env, tcg_constant_i32(tcg_ctx, reg));
            gen_op_mov_reg_v(s, ot, rm, s->T0);
        }
        break;
    ...
    case 0x123: /* mov drN, reg */
        if (check_cpl0(s)) {
            ...
            if (b & 2) {
                ...
                gen_helper_set_dr(tcg_ctx, cpu_env, s->tmp2_i32, s->T0);
                s->base.is_jmp = DISAS_EOB_NEXT;
            } else {
                gen_svm_check_intercept(s, SVM_EXIT_READ_DR0 + reg);
                tcg_gen_movi_i32(tcg_ctx, s->tmp2_i32, reg);
                gen_helper_get_dr(tcg_ctx, s->T0, cpu_env, s->tmp2_i32);
                gen_op_mov_reg_v(s, ot, rm, s->T0);
            }
        }
        break;
    ...
    case 0x1ae:
        ...
        CASE_MODRM_MEM_OP(3): /* stmxcsr */
            ...
            gen_helper_update_mxcsr(tcg_ctx, cpu_env);
            gen_lea_modrm(env, s, modrm);
        ...
        case 0xc8: case 0xc9: case 0xca: case 0xcb: case 0xcc: case 0xcd: case 0xce: case 0xcf: /* rdgsbase (f3 0f ae /1) */
        case 0xd0: case 0xd1: case 0xd2: case 0xd3: case 0xd4: case 0xd5: case 0xd6: case 0xd7: /* wrfsbase (f3 0f ae /2) */
        case 0xd8: case 0xd9: case 0xda: case 0xdb: case 0xdc: case 0xdd: case 0xde: case 0xdf: /* wrgsbase (f3 0f ae /3) */
            if (CODE64(s)
                && (prefixes & PREFIX_REPZ)
                && !(prefixes & PREFIX_LOCK)
    ...
    case 0x1b8: /* SSE4.2 popcnt */
        ...
        break;
    case 0x10e: case 0x10f: case 0x110: case 0x111: case 0x112: case 0x113: case 0x114: case 0x115: case 0x116: case 0x117:
...
void tcg_x86_init(struct uc_struct *uc)
{
    ...
    cpu_cc_op = tcg_global_mem_new_i32(tcg_ctx, cpu_env,
                                       offsetof(CPUX86State, cc_op), "cc_op");
    cpu_cc_dst = tcg_global_mem_new(tcg_ctx, cpu_env, offsetof(CPUX86State, cc_dst),
                                    "cc_dst");
    cpu_cc_src = tcg_global_mem_new(tcg_ctx, cpu_env, offsetof(CPUX86State, cc_src),
                                    "cc_src");
    cpu_cc_src2 = tcg_global_mem_new(tcg_ctx, cpu_env, offsetof(CPUX86State, cc_src2),
                                     "cc_src2");
    cpu_eip = tcg_global_mem_new(tcg_ctx, cpu_env, offsetof(CPUX86State, eip), eip_name);
    ...
    for (i = 0; i < 6; ++i) {
        cpu_seg_base[i]
            = tcg_global_mem_new(tcg_ctx, cpu_env,
    ...
    for (i = 0; i < 4; ++i) {
        cpu_bndl[i]
            = tcg_global_mem_new_i64(tcg_ctx, cpu_env,
...
static void i386_sync_pc(DisasContextBase *dcbase, CPUState *cpu)
{
    DisasContext *dc = container_of(dcbase, DisasContext, base);
                          ...
                          target_ulong *data)
{
    ...
    if (TARGET_TB_PCREL) {
        target_ulong pc = env->eip + tb->cs_base;
