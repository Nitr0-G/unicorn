...
static uint32_t test_arm_m33_sau_tt_query(uc_engine *uc, uint32_t address,
                                          uint8_t op)
{
    uint32_t result = 0;
    uint8_t code[] = {
...
static void test_arm_m33_sau_region(CPUARMState *env, uint32_t region,
...
static void test_arm_m33_sau_tt(void)
{
    ...
    result = test_arm_m33_sau_tt_query(uc, 0x2000, 0);
    TEST_CHECK(result & (1U << 22));
    TEST_CHECK(!(result & (1U << 17)));
    ...
    env->sau.ctrl = 1;
    test_arm_m33_sau_region(env, 0, 0x2000, 0x2fff, false);
    result = test_arm_m33_sau_tt_query(uc, 0x2000, 0);
    TEST_CHECK(!(result & (1U << 22)));
    TEST_CHECK(result & (1U << 17));
    TEST_CHECK(((result >> 8) & 0xff) == 0);

    test_arm_m33_sau_region(env, 1, 0x3000, 0x3fff, true);
    result = test_arm_m33_sau_tt_query(uc, 0x3000, 0);
    TEST_CHECK(result & (1U << 22));
    TEST_CHECK(result & (1U << 17));
    TEST_CHECK(((result >> 8) & 0xff) == 1);

    test_arm_m33_sau_region(env, 1, 0x2000, 0x2fff, true);
    result = test_arm_m33_sau_tt_query(uc, 0x2000, 0);
    TEST_CHECK(result & (1U << 22));
    TEST_CHECK(!(result & (1U << 17)));

    result = test_arm_m33_sau_tt_query(uc, 0xe000e010, 0);
    TEST_CHECK(result & (1U << 22));

    result = test_arm_m33_sau_tt_query(uc, 0xe000e010, 2);
    TEST_CHECK(!(result & (1U << 22)));

    OK(uc_close(uc));
}

static void test_arm_m55_pmsav8_pxn(void)
{
    uc_engine *uc;
    ARMCPU *cpu;
    CPUARMState *env;
    ...
    env->pmsav8.rbar[M_REG_S][0] = (uint32_t)code_start | (1U << 1);
    env->pmsav8.rlar[M_REG_S][0] =
        ((uint32_t)(code_start + code_len - 1) & ~0x1fU) | (1U << 4) | 1U;
...
static void test_arm_m_profile_activate_fp_context(uc_engine *uc)
{
    ...
    env->v7m.control[M_REG_S] |= control_fpca | control_sfpa;
    env->v7m.cpacr[M_REG_S] |= 3U << 20;
    env->v7m.fpccr[M_REG_S] &= ~R_V7M_FPCCR_LSPACT_MASK;
    env->v7m.fpccr[M_REG_NS] &= ~R_V7M_FPCCR_LSPACT_MASK;
    FIELD_DP32(env->hflags, TBFLAG_M32, LSPACT, 0, env->hflags);
    FIELD_DP32(env->hflags, TBFLAG_M32, NEW_FP_CTXT_NEEDED, 0,
               env->hflags);
    FIELD_DP32(env->hflags, TBFLAG_M32, FPCCR_S_WRONG, 0, env->hflags);
}

static void test_arm_m_profile_disable_lazy_fp(uc_engine *uc)
{
    ARMCPU *cpu = ARM_CPU(uc->cpu);
    CPUARMState *env = &cpu->env;

    env->v7m.fpccr[M_REG_S] &= ~R_V7M_FPCCR_LSPEN_MASK;
}
...
static void test_arm_m55_vlldm_vlstm(void)
{
    const uint32_t xpsr_t = 1U << 24;
    const uint32_t eci_a0a1 = 2U << 12;
    const uint32_t epsr_condexec_mask = 0xfc00 | (3U << 25);
    ...
    uint8_t mem[0x48] = { 0 };
    uint32_t r1 = (uint32_t)data_addr;
    uint32_t s0 = 0x11223344;
    uint32_t s1 = 0x55667788;
    uint32_t fpscr = 0x90000000;
    uint32_t vpr = 0x00abcdef;
    uint32_t epsr;
    uc_err err;

    test_arm_emit32(code, 0, 0x0a00ec21); /* vlstm r1, t1 */
    uc_common_setup(&uc, UC_ARCH_ARM,
                    UC_MODE_THUMB | UC_MODE_MCLASS,
                    (const char *)code, sizeof(code),
                    UC_CPU_ARM_CORTEX_M55);
    ...
    test_arm_m_profile_disable_lazy_fp(uc);
    OK(uc_mem_write(uc, data_addr, mem, sizeof(mem)));
    OK(uc_reg_write(uc, UC_ARM_REG_R1, &r1));
    OK(uc_reg_write(uc, UC_ARM_REG_S0, &s0));
    OK(uc_reg_write(uc, UC_ARM_REG_S1, &s1));
    ...
    OK(uc_mem_read(uc, data_addr, mem, sizeof(mem)));
    OK(uc_reg_read(uc, UC_ARM_REG_EPSR, &epsr));
    TEST_CHECK(test_arm_load_le(mem, 4) == s0);
    TEST_CHECK(test_arm_load_le(mem + 4, 4) == s1);
    TEST_CHECK((test_arm_load_le(mem + 0x40, 4) & 0xf0000000) ==
               (fpscr & 0xf0000000));
    TEST_CHECK(test_arm_load_le(mem + 0x44, 4) == vpr);
    ...
    fpscr = 0x60000000;
    vpr = 0x00123456;
    test_arm_emit32(code, 0, 0x0a80ec21); /* vlstm r1, t2 */
    uc_common_setup(&uc, UC_ARCH_ARM,
                    UC_MODE_THUMB | UC_MODE_MCLASS,
                    (const char *)code, sizeof(code),
                    UC_CPU_ARM_CORTEX_M55);
    ...
    memset(mem, 0, sizeof(mem));
    test_arm_store_le(mem, 4, 0x0badc0de);
    test_arm_store_le(mem + 4, 4, 0xf00d1234);
    test_arm_store_le(mem + 0x40, 4, 0xa0000000);
    test_arm_store_le(mem + 0x44, 4, 0x00fedcba);
    s0 = 0;
    s1 = 0;
    fpscr = 0;
    vpr = 0;
    test_arm_emit32(code, 0, 0x0a80ec31); /* vlldm r1, t2 */
    uc_common_setup(&uc, UC_ARCH_ARM,
                    UC_MODE_THUMB | UC_MODE_MCLASS,
                    (const char *)code, sizeof(code),
                    UC_CPU_ARM_CORTEX_M55);
    ...
    OK(uc_reg_read(uc, UC_ARM_REG_VPR, &vpr));
    TEST_CHECK(s0 == 0x0badc0de);
    TEST_CHECK(s1 == 0xf00d1234);
    TEST_CHECK((fpscr & 0xf0000000) == 0xa0000000);
    TEST_CHECK(vpr == 0x00fedcba);
    OK(uc_close(uc));
...
static void test_arm_m55_fpcxt_sysreg(void)
{
    const uint32_t control_fpca = 1U << 2;
    const uint32_t control_sfpa = 1U << 3;
    const uint32_t fpcr_v = 1U << 28;
    ...
    const uint32_t fpcr_z = 1U << 30;
    const uint32_t fpcr_n = 1U << 31;
    const uint32_t nzcv_mask = fpcr_n | fpcr_z | fpcr_c | fpcr_v;
    const uint64_t data_addr = code_start + 0x1000;
    const uint32_t fpdscr_ns = 0x00070000;
    uc_engine *uc;
    ARMCPU *cpu;
    CPUARMState *env;
    uint8_t code[8];
    uint8_t mem[16] = { 0 };
    uint32_t fpscr_initial = 0x0a040000 | fpcr_n | fpcr_c;
    uint32_t fpcxt_write = 0x86040000;
    uint32_t expected_write = fpcxt_write & ~nzcv_mask;
    uint32_t expected_read;
    uint32_t r1 = 0;
    uint32_t r2 = fpcxt_write;
    uint32_t r3 = 0;
    uint32_t fpscr = fpscr_initial;
    uint32_t rbase = (uint32_t)data_addr;
    uc_err err;
    ...
    cpu = ARM_CPU(uc->cpu);
    env = &cpu->env;
    env->v7m.fpdscr[M_REG_NS] = fpdscr_ns;
    OK(uc_reg_write(uc, UC_ARM_REG_FPSCR, &fpscr));
    OK(uc_reg_write(uc, UC_ARM_REG_R2, &r2));
    OK(uc_reg_read(uc, UC_ARM_REG_FPSCR, &fpscr));
    ...
    OK(uc_reg_read(uc, UC_ARM_REG_R3, &r3));
    OK(uc_reg_read(uc, UC_ARM_REG_FPSCR, &fpscr));
    TEST_CHECK_(r3 == expected_read,
                "r3=0x%08x", r3);
    TEST_CHECK_((env->v7m.control[M_REG_S] & control_sfpa) != 0,
                "control=0x%08x", env->v7m.control[M_REG_S]);
    TEST_CHECK_((fpscr & ~nzcv_mask) == expected_write,
                "fpscr=0x%08x expected=0x%08x", fpscr, expected_write);
    ...
    env->v7m.control[M_REG_S] &= ~control_fpca;
    fpscr = 0x01030000;
    OK(uc_reg_write(uc, UC_ARM_REG_FPSCR, &fpscr));
    OK(uc_reg_read(uc, UC_ARM_REG_FPSCR, &fpscr));
    expected_read = fpscr & ~nzcv_mask;
    OK(uc_reg_write(uc, UC_ARM_REG_R0, &fpcxt_write));
    OK(uc_emu_start(uc, code_start | 1, code_start + sizeof(code), 0, 0));
    OK(uc_reg_read(uc, UC_ARM_REG_R1, &r1));
    OK(uc_reg_read(uc, UC_ARM_REG_FPSCR, &fpscr));
    TEST_CHECK_(r1 == fpdscr_ns, "r1=0x%08x", r1);
    TEST_CHECK_((fpscr & ~nzcv_mask) == expected_read,
                "fpscr=0x%08x", fpscr);
    TEST_CHECK_((env->v7m.control[M_REG_S] & control_fpca) == 0,
                "control=0x%08x", env->v7m.control[M_REG_S]);
    OK(uc_close(uc));
    ...
    env = &cpu->env;
    env->v7m.fpdscr[M_REG_NS] = fpdscr_ns;
    env->v7m.control[M_REG_S] &= ~control_sfpa;
    fpscr = fpscr_initial;
    OK(uc_reg_write(uc, UC_ARM_REG_FPSCR, &fpscr));
    OK(uc_reg_read(uc, UC_ARM_REG_FPSCR, &fpscr));
    expected_read = fpscr & ~nzcv_mask;
    ...
    OK(uc_reg_read(uc, UC_ARM_REG_R1, &r1));
    OK(uc_reg_read(uc, UC_ARM_REG_FPSCR, &fpscr));
    TEST_CHECK_(r1 == expected_read, "r1=0x%08x", r1);
    TEST_CHECK_((env->v7m.control[M_REG_S] & control_sfpa) != 0,
                "control=0x%08x", env->v7m.control[M_REG_S]);
    TEST_CHECK_((fpscr & ~nzcv_mask) == expected_write,
                "fpscr=0x%08x expected=0x%08x", fpscr, expected_write);
    ...
    test_arm_emit32(code, 0, 0xef80edc1); /* vstr fpcxt_s,[r1,#0] */
    uc_common_setup(&uc, UC_ARCH_ARM,
                    UC_MODE_THUMB | UC_MODE_MCLASS,
    ...
    fpscr = fpscr_initial;
    OK(uc_mem_write(uc, data_addr, mem, sizeof(mem)));
    OK(uc_reg_write(uc, UC_ARM_REG_R1, &rbase));
    OK(uc_reg_write(uc, UC_ARM_REG_FPSCR, &fpscr));
    OK(uc_reg_read(uc, UC_ARM_REG_FPSCR, &fpscr));
    expected_read = (fpscr & ~nzcv_mask) | 0x80000000;
    OK(uc_emu_start(uc, code_start | 1, code_start + 4, 0, 0));
    ...
    TEST_CHECK_((env->v7m.control[M_REG_S] & control_sfpa) == 0,
                "control=0x%08x", env->v7m.control[M_REG_S]);
    TEST_CHECK_((fpscr & ~nzcv_mask) == fpdscr_ns,
                "fpscr=0x%08x", fpscr);
    OK(uc_close(uc));

    test_arm_emit32(code, 0, 0xef80edd1); /* vldr fpcxt_s,[r1,#0] */
    uc_common_setup(&uc, UC_ARCH_ARM,
                    UC_MODE_THUMB | UC_MODE_MCLASS,
                    (const char *)code, 4, UC_CPU_ARM_CORTEX_M55);
    test_arm_enable_vfp(uc);
    ...
    test_arm_emit32(code, 0, 0xcf80edc1); /* vstr fpcxt_ns,[r1,#0] */
    uc_common_setup(&uc, UC_ARCH_ARM,
                    UC_MODE_THUMB | UC_MODE_MCLASS,
                    (const char *)code, 4, UC_CPU_ARM_CORTEX_M55);
    test_arm_enable_vfp(uc);
    ...
    env->v7m.control[M_REG_S] &= ~control_fpca;
    fpscr = 0x04460000;
    r2 = (uint32_t)data_addr;
    test_arm_store_le(mem, 4, fpcxt_write);
    OK(uc_mem_write(uc, data_addr, mem, sizeof(mem)));
    OK(uc_reg_write(uc, UC_ARM_REG_R2, &r2));
    OK(uc_reg_write(uc, UC_ARM_REG_FPSCR, &fpscr));
    OK(uc_reg_read(uc, UC_ARM_REG_R2, &r2));
    TEST_CHECK_(r2 == (uint32_t)data_addr, "initial r2=0x%08x", r2);
    OK(uc_reg_read(uc, UC_ARM_REG_FPSCR, &fpscr));
    expected_read = fpscr & ~nzcv_mask;
    OK(uc_emu_start(uc, code_start | 1, code_start + 4, 0, 0));
    OK(uc_reg_read(uc, UC_ARM_REG_R2, &r2));
    OK(uc_reg_read(uc, UC_ARM_REG_FPSCR, &fpscr));
    TEST_CHECK_(r2 == (uint32_t)data_addr + 4, "r2=0x%08x", r2);
    TEST_CHECK_((fpscr & ~nzcv_mask) == expected_read,
                "fpscr=0x%08x", fpscr);
    OK(uc_close(uc));
    ...
    OK(uc_reg_read(uc, UC_ARM_REG_FPSCR, &fpscr));
    TEST_CHECK_(r2 == (uint32_t)data_addr + 4, "r2=0x%08x", r2);
    TEST_CHECK_(test_arm_load_le(mem, 4) == fpdscr_ns,
                "stored=0x%08x", test_arm_load_le(mem, 4));
    TEST_CHECK_((fpscr & ~nzcv_mask) == expected_read,
                "fpscr=0x%08x", fpscr);
    OK(uc_close(uc));
    ...
    env->v7m.fpccr[M_REG_NS] &= ~R_V7M_FPCCR_ASPEN_MASK;
    env->v7m.cpacr[M_REG_S] &= ~(3U << 20);
    env->v7m.cpacr[M_REG_NS] &= ~(3U << 20);
    FIELD_DP32(env->hflags, TBFLAG_ANY, FPEXC_EL, 1, env->hflags);
    FIELD_DP32(env->hflags, TBFLAG_M32, LSPACT, 0, env->hflags);
    err = uc_emu_start(uc, code_start | 1, code_start + 4, 0, 0);
    TEST_CHECK_(err == UC_ERR_EXCEPTION, "err=%u", (unsigned)err);
    OK(uc_close(uc));
...
TEST_LIST = {{"test_arm_nop", test_arm_nop},
             ...
             {"test_arm_m55_pmsav8_pxn", test_arm_m55_pmsav8_pxn},
             ...
             {"test_arm_m55_vlldm_vlstm", test_arm_m55_vlldm_vlstm},
             {"test_arm_m55_fpcxt_sysreg", test_arm_m55_fpcxt_sysreg},
